...
public class Customer implements ICustomer {
    ...
    public List<Ticket> searchTicket(Date date, int route) throws RuntimeException {
        ...
        for (Ticket ticket : list) {
            if (ticket.getDate().equals(date) && ticket.getRouteNumber()==route) { // Тут добавил сравнение по
                // маршруту, потому что непонятно, зачем нам список всех маршрутов, если мы указали конкретный
                // маршрут. !!!
                result.add(ticket);
            }
        }
